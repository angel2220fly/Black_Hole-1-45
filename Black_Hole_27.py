...
def compress_file_with_words_and_spaces(dictionary_file, input_filename, output_filename, encoding="utf-8"):
    ...
    try:
        ...
        words = data.split(b" ")  # Split the binary data by spaces
        ...
        for word in words:
            try:
                ...
                if decoded_word in word_to_index:
                    ...
                    if index > 3:  # Only encode if the index is greater than 3
                        ...
                        leading_zeros = len(word) - len(word.lstrip(b'\0'))
                        if leading_zeros > 31:  # Limiting to 5 bits (32 possible values)
                            leading_zeros = 31
                        encoded_data.append(leading_zeros)  # Append leading zeros (5 bits)
                        word = word.lstrip(b'\0')  # Remove leading zeros
            ...
            # Handle space
            encoded_data.append(0x20)  # Add a space delimiter after each word
        ...
        with open(output_filename, "wb") as outfile:
            ...
            print(f"File compressed using dictionary-based compression and PAQ, saved as '{output_filename}'")
    ...
    try:
        ...
        while i < len(decompressed_data):
            ...
            if flag == 0x00:  # Dictionary word
                leading_zeros = decompressed_data[i]  # Read leading zeros (5 bits)
                i += 1
                index = struct.unpack(">I", decompressed_data[i:i+4])[0]
                word = index_to_word.get(index, "<unknown>").encode(encoding)

                # Apply leading zeros back to the word
                word = b'\0' * leading_zeros + word
...
def main():
    ...
    if choice == '1':
        ...
        compress_file_with_words_and_spaces("Dictionary.txt", input_file, output_file)
    elif choice == '3':
